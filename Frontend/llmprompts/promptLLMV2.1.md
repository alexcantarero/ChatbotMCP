...
3. **get-airport-info**: Use this tool to retrieve detailed information about a specific airport (name, location, city, country, terminals, etc.). 
   ...
   - **Important**: If the user mentions a city name instead of an airport code, identify the main airport IATA code for that city. Common Spanish airports:
     ...
     * Málaga → AGP
     * Bilbao → BIO 
    There are instructions on what to provide to the tool so it does the job for you.
...
- **get-hotel-near-to-place**: Requires latitude, longitude, and radius
- **get-activities-near-place**: Requires latitude and longitude
